...
def display_ls_metrics(scheme, df, load_profile):
    ...
    zone_MD = {zone: load_profile_metric(
        df=load_profile, zone=zone) for zone in zones}
...
# def remove_duplicates_keep_nan(df, columns):
#     is_duplicate = df[columns].duplicated()
#     st.write("df")
#     st.write(df)
#     st.write("is_duplicate")
#     st.write(is_duplicate)
#     is_nan = df[columns].isna()
...
def remove_duplicates_keep_nan(df, columns):
    ...
    df_clean = pd.concat(
        [
            df.loc[mask].drop_duplicates(subset=columns),
            df.loc[~mask]
        ]
    ).sort_index()
