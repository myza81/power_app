...
class LoadShedding:
    ...
    def dp_grpId_loadquantum(self) -> pd.DataFrame:
        ...
        load["Pload (MW)"] = pd.to_numeric(
            load["Pload (MW)"], errors='coerce').fillna(0)
        load["Qload (Mvar)"] = pd.to_numeric(
            load["Qload (Mvar)"], errors='coerce').fillna(0)

        load_grouped = (
            load.groupby(
                ["mnemonic", "kV", "local_trip_id", "group_trip_id", "assignment_id",
                    "ls_dp", "critical_list", "short_text", "long_text"],
                as_index=False,
            )
            .agg(
                {
    ...
    def automatic_loadshedding_rly(self):
        ...
        if self.hvcb_rly_loc is None and self.incomer_rly_loc is None:
            return pd.DataFrame()

        required_columns = ['local_trip_id', 'mnemonic',
                            'assignment_id', 'feeder_id', 'breaker_id']
        hvcb_dp = self.ls_hvcb.copy(deep=True)
        hvcb_dp['assignment_id'] = hvcb_dp['group_trip_id']
        hvcb_rly = hvcb_dp.groupby(
            ['group_trip_id', 'mnemonic', 'local_trip_id', 'assignment_id'], as_index=False
        ).agg(
            {
                'feeder_id': lambda x: ", ".join(x.astype(str).unique()),
                'breaker_id': lambda x: ", ".join(x.astype(str).unique())
        ...
        available_rly_with_load = pd.merge(
            ...
            on=["mnemonic", "local_trip_id",
                "assignment_id", "feeder_id", "breaker_id"],
            how="left",
        )

        available_rly_meta = pd.merge(
            available_rly_with_load,
            self.subs_metadata_enrichment(),
            on="mnemonic", how="left")

        return available_rly_meta
